function occurenceOfSubstr(s: string, occurence: string) {
  if (s.length < occurence.length) {
    return 0;
  }

  if (s.startsWith(occurence)) {
    return 1 + occurenceOfSubstr(s.slice(occurence.length), occurence);
  } else {
    return occurenceOfSubstr(s.slice(1), occurence);
  }
}

function countNumLettSymb(str: string) {
  function countNumLettSymb(str: string) {
...
function decompressString(s: string): string {
  ...
  for (let i = 0; i < s.length; i++) {
      if(/[a-zA-z]/.test(s[i])) {
          if (/[0-9]/.test(s[i+1])) {
              result.push(s[i].repeat(Number(s[i+1])));
          }
      }
  }
  return result.reduce(((acc, e) => acc += e), "");;
}
...
/**[2] */
function isAnagrams(s: string, t: string): boolean {
  s = t.split("").sort().join("");
  t = t.split("").sort().join("");
  return s === t;
}
...
/**[4] */
function reverseWordsWithoutExtraSpace(s: string) {
  const newString = s
    .split(" ")
    .reverse()
    .filter((word) => word !== "")
...
function longestSubstringWithoutRepeatingChars(s: string): string {
...
function distributeSpacesInline(
  inlineWords: string[],
  charCountOfInlineWords: number,
  maxWidth: number
): string {
...
function leftJustifyLine(inlineWords: string[], maxWidth: number): string {
...
function fullJustify(words: string[], maxWidth: number): string[] {
  ...
  words.forEach((word) => {
    ...
    if (charCountOfInlineWords + word.length + inlineWords.length > maxWidth) {
      res.push(
        this.distributeSpacesInline(
          inlineWords,
          charCountOfInlineWords,
          maxWidth
        )
      );
...
function getVal(char: string, arr: [string, number][]): number {
...
function fullJustify2(words: string[], maxWidth: number): string[] {
  let res: string[] = [];
  let line: string[] = [];
