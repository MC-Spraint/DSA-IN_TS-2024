...
function arithmeticSequenceSum(firstTerm: number, n: number, commonDifference: number): number {
...
function printPrimes(n: number): number[] {
    ...
    while (arr.length < n) {
        let count = 0;
        for (let i = 2; i <= cur - 1; i++) { // Optimize to sqrt(num)
            if (cur % i === 0) {
                count++;
            }
        }
        if (count === 0 && cur >= 2) //edge case
            arr.push(cur); 
        cur++;
    }
    return arr;
...
function applyConstraints(reversed: number): number {
